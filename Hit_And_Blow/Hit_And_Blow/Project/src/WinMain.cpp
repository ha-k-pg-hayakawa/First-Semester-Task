	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		if(gameClear==false)
		{
			if(IsPushKey(MY_INPUT_UP))
			{
				...
				if (num[cursor] >= 10)num[cursor] = 0;	// 増やした結果、9 より大きくなった場合は、0にする
			}
			else if(IsPushKey(MY_INPUT_DOWN))
			{
				num[cursor]--;	// num配列のcursor番目を 1 減らす
				if (num[cursor] < 0)num[cursor] = 9;	// 減らした結果、0 より小さくなった場合は、9にする
			}
			else if(IsPushKey(MY_INPUT_LEFT))
			{
				cursor--;//※※	// cursorを 1 減らす
				if (cursor < 0)cursor = 0;	// 減らした結果、0 より小さくなった場合は、0にする
			}
			else if(IsPushKey(MY_INPUT_RIGHT))
			{
				cursor++;	// cursorを 1 増やす
				if (cursor > digits - 1)cursor = digits - 1;		// 増やした結果、(DIGITS - 1) より大きくなった場合は、(DIGITS - 1)にする
			}
